...
- Datasets and model variations have immutable numbered versions with notes, carry-over of unchanged files, a file browser with safe bounded previews (tables, JSON, text, images, NumPy arrays, ZIP listings; HTML/SVG/XML download-only), resumable chunked uploads for files of any size, and per-user storage quotas ([datasets](docs/datasets.md), [models](docs/models.md)). Models have Markdown model cards and framework variations. Notebooks pin a dataset or model version (or follow the latest) in interactive sessions, Save & Run All and scheduled runs. Object storage and hosted inference are not implemented.
