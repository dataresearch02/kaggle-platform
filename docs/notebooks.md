...
Dataset and model sources added from the input catalog are copied under `input/<source-folder>/` and pin the source's newest version; the input panel shows the version and lets you choose another version or **Always latest**. Save & Run All and scheduled runs stage the same version read-only, and sources too large for an interactive session (over 200 MB or 1,000 files) are staged only there. See [datasets](datasets.md#notebooks) and [models](models.md#using-a-model-in-a-notebook).

**Save Version** stores an immutable version in the history, with sharing controls alongside. Choose CPU or GPU next to **Start session**; a GPU session holds a whole GPU until it stops, so the editor shows a warning with a stop button. The notebook panel shows the session accelerator, "Internet: off", the weekly GPU usage meter, **Background runs** (Save & Run All from the latest saved version, run history with logs and cancellation) and **Schedule a notebook to run** (daily, weekly or every N hours in UTC). See [compute](compute.md).
